...
from minie_process import Popen, PIPE, STDOUT
...
class GnuFCompiler(FCompiler):
    ...
    def get_libgcc_dir(self):
        try:
            output = minie_process.check_output(self.compiler_f77 +
                                                ['-print-libgcc-file-name'])
        except (OSError, minie_process.CalledProcessError):
            pass
        else:
...
class Gnu95FCompiler(GnuFCompiler):
    ...
    def get_target(self):
        try:
            output = minie_process.check_output(self.compiler_f77 + ['-v'])
        except (OSError, minie_process.CalledProcessError):
            pass
        else:
            output = filepath_from_subprocess_output(output)
