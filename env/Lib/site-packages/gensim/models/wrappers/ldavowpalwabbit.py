...
def _run_vw_command(cmd):
    ...
    proc = minie_process.Popen(cmd, stdout=minie_process.PIPE,
                               stderr=minie_process.STDOUT)
    output = proc.communicate()[0].decode('utf-8')
    logger.debug("Vowpal Wabbit output: %s", output)

    if proc.returncode != 0:
        raise minie_process.CalledProcessError(proc.returncode, ' '.join(cmd), output=output)
